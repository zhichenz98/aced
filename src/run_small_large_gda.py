...
def main_gda(train_set='small', test_set='large', train_idx=0, test_idx=5, train_sig=0.01, test_sig=0.01, train_bias=0.00, test_bias=0.00, apply_da=True, lambda_da=1, num_conv=1, num_steps=3, num_graphs=100):
    ...
    ## generate intermediate graphs
    print('Generating intermediate graphs...')
    lams = [1./num_steps for _ in range(num_steps)]
    mixed_graphs = get_intermediate_graphs(train_graph, test_graph, num_graphs=num_graphs, lams=lams)
    last_graph = copy.deepcopy(test_graph)
    mixed_graphs = [train_graph] + mixed_graphs + [last_graph]
    ...
    for i in range(num_steps+1):
        train_loader = DataLoader(mixed_graphs[i], batch_size=64, shuffle=True)
        test_loader = DataLoader(mixed_graphs[i+1], batch_size=64, shuffle=True)
        ...
        model.eval()
        with torch.no_grad():
            for g in mixed_graphs[i+1]:  # 逐个图处理
                g = g.to(device)
                out = model(g)  # (num_nodes, d)
                out = out * g.x[:, -1].reshape(-1, 1)
                g.y = out.cpu()  # 把预测值写回每个图
    ...
    for idx in range(len(mixed_graphs[-1])):  # 逐个图处理
        g = mixed_graphs[-1][idx]  # 获取图
        out = model(g)  # (num_nodes, d)
        out = out * g.x[:, -1].reshape(-1, 1)
        test_out.append(out.detach())
        test_y.append(test_graph[idx].y)
...
for test_idx in test_idx_list:
    for test_sig in test_sig_list:
        for test_bias in test_bias_list:
            ...
            for train_idx in train_idx_list:
                for train_sig in train_sig_list:
                    for train_bias in train_bias_list:
                        tmp_train_metric, tmp_test_metric = main_gda(train_set, test_set, train_idx, test_idx, train_sig, test_sig, train_bias, test_bias, apply_da=apply_da, lambda_da=lambda_da, num_conv=num_conv, num_graphs=num_graphs, num_steps=num_steps)
            ...
            if apply_da:
                np.savetxt('result/gda/{}2{}/{:d}_{:.2f}_{:.2f}_da.csv'.format(train_set, test_set, test_nodes, test_idx, test_sig, test_bias), metric, fmt='%s', delimiter=',')
            else:
                np.savetxt('result/gda/{}2{}/{:d}_{:.2f}_{:.2f}.csv'.format(train_set, test_set, test_nodes, test_idx, test_sig, test_bias), metric, fmt='%s', delimiter=',')
