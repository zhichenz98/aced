...
class KKTLoss(nn.Module):
    ...
    def forward(
        ...
        KKT_error += ((Volt.T * (Y @ Volt.T)).T.view(-1, 2, n_bus).sum(dim=1) + (PQ_Loads - PQ_Gens)[:, :n_bus]).abs().sum(1)
        KKT_error += ((Volt.T * (Yconj @ Volt.T)).T.view(-1, 2, n_bus).sum(dim=1) + (PQ_Loads - PQ_Gens)[:, n_bus:]).abs().sum(1)
