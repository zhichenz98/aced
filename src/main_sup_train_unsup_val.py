...
def main(args):
    ...
    if args.optimizer == 'adam':
        sup_optimizer = optim.Adam(model.parameters(), lr=args.lr)
        unsup_optimizer = optim.Adam(model.parameters(), lr=args.lr*0.001)
    elif args.optimizer == 'sgd':
        sup_optimizer = optim.SGD(model.parameters(), lr=args.lr)
        unsup_optimizer = optim.SGD(model.parameters(), lr=args.lr*0.001)
    elif args.optimizer == 'adamw':
        sup_optimizer = optim.AdamW(model.parameters(), lr=args.lr)
        unsup_optimizer = optim.AdamW(model.parameters(), lr=args.lr*0.001)
    elif args.optimizer == 'adamw_wo_decay':
        sup_optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.0)
        unsup_optimizer = optim.AdamW(model.parameters(), lr=args.lr*0.001, weight_decay=0.0)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, Y) in enumerate(tqdm(dataloader_val)):
            ...
            if i == 0:
                tqdm.write(f'Epoch {epoch + 1}/{args.epochs}')
                tqdm.write(f'Val unsup_loss: {unsup_loss.item()}')
